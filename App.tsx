  ...
  const triggerAnnouncement = useCallback(async (type: 'hourly' | 'jingle', force = false, overrideText?: string) => {
    ...
    const performRequest = async (): Promise<string | undefined> => {
      ...
      if (overrideText) {
        ...
        // Seleccionar scripts según día y hora
        const hour = now.getHours();
        const dayOfWeek = now.getDay(); // 0=domingo, 6=sábado
        const isMadrugada = hour >= 0 && hour < 5;
        const isSchoolMorning = hour >= 5 && hour < 8 && dayOfWeek >= 1 && dayOfWeek <= 5;
        const isSaturdayNight = dayOfWeek === 6 && hour >= 18 && hour < 22;
        const isSunday = dayOfWeek === 0 && !isMadrugada;
        const scripts = isMadrugada
          ? MADRUGADA_SCRIPTS
          : isSchoolMorning
